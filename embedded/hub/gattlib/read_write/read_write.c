...
static void on_device_connect(gattlib_adapter_t* adapter, const char *dst, gattlib_connection_t* connection, int error, void* user_data) {
	...
	if (m_argument.operation == READ) {
		...
		ret = gattlib_write_char_by_uuid(connection, &m_argument.uuid, m_argument.value_data, strlen(m_argument.value_data));
		if (ret != GATTLIB_SUCCESS) {
			char uuid_str[MAX_LEN_UUID_STR + 1];
...
int set_ble(int argc, char **argv) {
	...
	if (strcmp(argv[2], "read") == 0) {
		...
		if ((strlen(argv[4]) >= 2) && (argv[4][0] == '0') && ((argv[4][1] == 'x') || (argv[4][1] == 'X'))) {
			memset(m_argument.value_data, 0, 100);
			strncpy(m_argument.value_data, argv[4], 100);
		} else {
			memset(m_argument.value_data, 0, 100);
			strncpy(m_argument.value_data, argv[4], 100);
		}
